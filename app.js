...
const storage = new Storage({
  ...
  keyFilename: path.join(
    __dirname,
    "cloud",
    "certain-router-414905-f384a959b654.json"
  ),
});
...
app.post("/nuevo_negocio", (req, res, next) => {
  const { s_nombre_empresa } = req.body;
  ...
  db.none(
    ...
    .then((data) => {
      ...
      res
        .status(200)
        .json({ message: "Negocio insertado correctamente", status: true });
    })
    .catch((error) => {
      console.error("Error:", error);
      res.status(500).json({
        message: "Hubo un error al insertar el negocio",
        status: false,
      });
    });
});
...
app.post("/editar_negocio", (req, res, next) => {
  const { sk_empresa, s_nombre_empresa, s_color_primario, s_color_secundario } =
    req.body;
  ...
  db.none(
    ...
    WHERE sk_empresa = $4`,
    [s_nombre_empresa, s_color_primario, s_color_secundario, sk_empresa]
  )
    .then((data) => {
      console.log(data);
      res
        .status(200)
        .json({ message: "Negocio editado correctamente", status: true });
    })
    .catch((error) => {
      console.error("Error:", error);
      res
...
app.post("/nueva_licencia", (req, res, next) => {
  const { s_nombre, s_usuario, s_password, sk_empresa } = req.body;
  ...
  if (req.body.sk_licencia) {
    ...
    s_nombre = $1,
      ...
      .then((data) => {
        ...
        res
          .status(200)
          .json({ message: "Licencia modificada correctamente", status: true });
      })
      .catch((error) => {
        console.error("Error:", error);
        res.status(500).json({
          message: "Hubo un error al modificar la licencia.",
          status: false,
        });
      });
  } else {
    ...
    db.none(
      ...
      .then((data) => {
        ...
        res
          .status(200)
          .json({ message: "Licencia creada correctamente", status: true });
      })
      .catch((error) => {
        console.error("Error:", error);
        res.status(500).json({
          message: "Hubo un error al crear la licencia.",
          status: false,
        });
      });
  }
...
app.post("/eliminar_licencia", (req, res) => {
  ...
  db.none(`DELETE FROM cat_licencias WHERE sk_licencia = $1`, [sk_licencia])
    .then(() => {
      res
        .status(200)
        .json({ message: "Licencia eliminada correctamente", status: true });
    })
    .catch((error) => {
      console.error("Error:", error);
      res.status(500).json({
        message: "Hubo un error al eliminar la licencia",
        status: false,
      });
    });
});
...
app.get("/obtenerClientes", (req, res) => {
  db.any(
    `SELECT * FROM cat_empresas WHERE i_administrador = 0 ORDER BY s_nombre_empresa ASC `
  )
    .then((data) => {
      res.json(data);
    })
...
app.post("/cambiarEstatusCliente", (req, res) => {
  const { sk_estatus, sk_empresa } = req.body;

  db.none("UPDATE cat_empresas SET sk_estatus = $1 WHERE sk_empresa = $2", [
    sk_estatus,
    sk_empresa,
  ])
    .then(() => {
      res.status(200).json({
        message: "Se modifico el estatus del cliente correctamente.",
        status: true,
      });
    })
    .catch((error) => {
      console.error("Error:", error);
      res.status(500).json({
        message: "Hubo un error al modificar el estatus del cliente",
        status: false,
      });
    });
});
...
app.post("/cambiarEstatusLicencia", (req, res) => {
  ...
  db.none("UPDATE cat_licencias SET sk_estatus = $1 WHERE sk_licencia = $2", [
    ...
    .then(() => {
      res.status(200).json({
        message:
          "Se modifico el estatus de la licencia del cliente correctamente.",
        estatus: true,
      });
    })
    .catch((error) => {
      console.error("Error:", error);
      res.status(500).json({
        message: "Hubo un error al modificar el estatus de la licencia",
        estatus: false,
      });
    });
});
  ...
  if (req.file) {
    ...
    db.none(
      ...
      .then(() => {
        res
          .status(200)
          .json({ message: "Usuario modificado correctamente", status: true });
      })
      .catch((error) => {
        console.error("Error:", error);
        res.status(500).json({
...
app.post("/renovacion_usuario", multer.single(), async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Renovación agregada correctamente", status: true });
  } catch (error) {
    console.error("Error:", error);
    res
      .status(500)
      .json({ message: "Hubo un error al renovar usuario", status: false });
  }
});
...
app.post("/eliminarUsuario", (req, res) => {
  ...
  db.none(
    ...
    .then(() => {
      res
        .status(200)
        .json({ message: "Usuario eliminado correctamente", status: true });
    })
    .catch((error) => {
      console.error("Error:", error);
      res.status(500).json({
...
app.get("/obtenerUsuarios/:sk_usuario", async (req, res) => {
  ...
  db.one(
    ...
    .catch((error) => {
      console.error("Error:", error);
      res.status(500).json({
        message: "Hubo un error al obtener información del usuario",
        status: false,
      });
    });
});
...
app.post("/eliminarFoto", async (req, res) => {
  ...
  try {
    const data = await db.one(
      `SELECT s_foto FROM cat_usuarios WHERE sk_usuario = '${sk_usuario}' `
    );
    ...
    await db.none(
      `UPDATE cat_usuarios SET s_foto = null WHERE sk_usuario = '${sk_usuario}' `
    );
    ...
    res
      .status(500)
      .json({ message: "Hubo un error al eliminar la foto", status: false });
  }
});
...
app.get("/detalle_usuario/:sk_usuario", (req, res) => {
  ...
  db.one(
    ...
    .then((data) => {
      let d_fecha_inscripcion = moment(data.d_fecha_inscripcion).format(
        "DD/MM/YYYY"
      );
      let d_fecha_renovacion = moment(data.d_fecha_renovacion).format(
        "DD/MM/YYYY"
      );
...
app.get("/obtenerEstadisticas", async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      message: "Analisis obtenidos correctamente",
      status: true,
    });
  } catch (error) {
    console.error("Error:", error);
...
async function verificarEstadoEmpresa(sk_empresa) {
  ...
  await db
    .one(`SELECT sk_estatus FROM cat_empresas WHERE sk_empresa = $1`, [
      sk_empresa,
    ])
    .then((data) => {
      if (data.sk_estatus === "AC") {
        result = {
          message: "La empresa si esta activa",
          success: true,
          empresa_cancelada: false,
        };
      } else {
        result = {
          message: "La empresa se encuentra INACTIVA",
          success: false,
          empresa_cancelada: true,
        };
      }
    })
    .catch((error) => {
...
async function verificarEstadoLicencia(sk_licencia) {
  ...
  await db
    .one(`SELECT sk_estatus FROM cat_licencias WHERE sk_licencia = $1`, [
      sk_licencia,
    ])
    .then((data) => {
      if (data.sk_estatus === "AC") {
        result = {
          message: "La licencia si esta activa",
          success: true,
          empresa_cancelada: false,
        };
      } else {
        result = {
          message: "La licencia se encuentra INACTIVA",
          success: false,
          empresa_cancelada: true,
        };
      }
